        ...
        try:
            ...
            try:
                ...
                daily_query = text("""
                    ...
                    LIMIT 1
                """)
                daily_result = session.execute(daily_query, {"cusno": user_id}).fetchone()
                ...
                monthly_query = text("""
                    ...
                    LIMIT 1
                """)
                monthly_result = session.execute(monthly_query, {"cusno": user_id}).fetchone()
                
                if not daily_result and not monthly_result:
                    return None
                ...
                if daily_result:
                    daily_dict = dict(daily_result._mapping)
                    # 메타데이터 컬럼 제외
                ...
                if monthly_result:
                    monthly_dict = dict(monthly_result._mapping)
                    # 메타데이터 컬럼 제외
