...
class SessionStatus(Base):
    ...
    session_id = Column(Integer, ForeignKey("sessions.session_id", ondelete="CASCADE"), primary_key=True, comment="세션 ID (FK)")
    ...
    # 대화 상태
    conversation_status = Column(SQLEnum(ConversationStatusEnum), default=ConversationStatusEnum.START, nullable=False, comment="대화 상태")

    # Task Queue 상태
    task_queue_status = Column(SQLEnum(TaskQueueStatusEnum), default=TaskQueueStatusEnum.NULL, nullable=False, comment="Task Queue 상태")

    # SubAgent 상태
    subagent_status = Column(SQLEnum(SubAgentStatusEnum), default=SubAgentStatusEnum.UNDEFINED, nullable=False, comment="SubAgent 상태")
    ...
    def __repr__(self):
        return f"<SessionStatus(session_id={self.session_id}, conversation={self.conversation_status}, subagent={self.subagent_status})>"
