...
def row_to_doc(row: list, cols: list) -> dict:
    ...
    try:
        ...
        # 라인 번호는 호출자가 제공해야 함
        if CSVParseError != Exception:
            raise CSVParseError(0, e)
        raise
...
class ProgressStream:
    ...
    def read(self, size: int = -1) -> bytes:
        ...
        if data:
            ...
            if current_time - self.last_activity_time > self.stall_timeout:
                ...
                if Logger:
                    Logger.warning(
                        f"{self.label} 업로드가 {elapsed_stall}초간 진행되지 않았습니다.",
                        details="네트워크 또는 서버 상태를 확인하세요."
                    )
                else:
                    print(f"경고: {self.label} 업로드가 {elapsed_stall}초간 진행되지 않았습니다.", 
    ...
    def _report(self):
        ...
        if Logger:
            ...
            print(f"[{self.label}] {percent:.1f}% ({self.uploaded:,}B / {self.total_size:,}B)", 
                  file=sys.stderr, flush=True)
    ...
    if key not in _tls.clients:
        try:
            ...
            if Config:
                timeout = urllib3.Timeout(
                    connect=Config.get_connect_timeout(),
            ...
            http = urllib3.PoolManager(
                ...
                retries=urllib3.Retry(
                    total=Config.get_retries() if Config else 3,
                    backoff_factor=1,
        ...
        try:
            _tls.clients[key] = (
                ...
                if http else Minio(host, access_key=access_key, secret_key=secret_key, secure=secure)
            )
        except Exception as e:
            if Logger:
                from batch_profile_exceptions import MinIOConnectionError
                raise MinIOConnectionError(host, e)
            raise
...
def validate_data_file(data_path: str, expected_columns: list) -> Tuple[int, Optional[int], str]:
    """
...
def get_columns_and_label(object_prefix: str) -> Tuple[list, str]:
    ...
    try:
        ...
        raise ImportError("batch_profile_schema 모듈을 찾을 수 없습니다.")
