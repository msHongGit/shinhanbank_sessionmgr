...
class TestSessionCreate:
    ...
    @pytest.mark.asyncio
    async def test_create_session_success_with_agw(self, client, agw_headers):
        """AGW가 세션 생성"""
        request_data = {
            "userId": "0616001905",
        }
    ...
    @pytest.mark.asyncio
    async def test_create_session_success_with_ma(self, client, ma_headers):
        """MA가 세션 생성 (인증 비활성화 상태에서 가능)"""
        request_data = {
            "userId": "user_002",
        }
        print("[TEST] POST /api/v1/sessions 요청:", request_data)
        response = await client.post("/api/v1/sessions", json=request_data, headers=ma_headers)
        print("[TEST] POST 응답:", response.status_code, response.json())
        assert response.status_code == 201
        data = response.json()
        assert data["global_session_key"].startswith("gsess_")
    ...
    @pytest.mark.asyncio
    async def test_create_session_without_profile(self, client, agw_headers):
        """프로파일 없는 사용자로 세션 생성"""
        request_data = {
    ...
    @pytest.mark.asyncio
    async def test_create_session_without_user_id(self, client, agw_headers):
