...
로그 수집 파이프라인(Filebeat → Logstash → ES Ingest Pipeline 등)에서 ESLOG를 읽어 `session_state` 인덱스를 갱신한다.
...
> TTL 만료 이벤트(키 만료 알림)를 사용하지 않는 대신, 
...
- 장점:
  ...
  - Redis keyspace notifications(TTL 만료 이벤트)를 사용하지 않아도 됨.
